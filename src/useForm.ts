...
const get = (key, storage = window.localStorage) => {
  const item = storage.getItem(key);
  // Parse stored json or if none return initialValue
  return item ? JSON.parse(item) : undefined;
}

const set = (key, value, storage = window.localStorage) => {
...
function useForm(stateSchema, validationSchema: any = {}, callback, options: any = {}) {
